...
- 面积工具形成三个以上顶点后，基于只读 `metric_transform` 自动计算预览面积，并将结果放在多边形视觉中心；已保存面积改用服务端 `metric_geometry` 计算中心显示值。
- 边长和面积标签移除不透明黑色矩形底框，改为浅色文字配细暗色描边；中心面积字号略大，边长字号略小，降低对车辆、道路和几何线的遮挡。
- 浏览器预览只用于交互反馈，提交内容仍为图像像素几何，服务端量算与版本链仍是最终事实。

### Verification

- [x] 自动化覆盖绘制中面积 `aria-live` 更新及画布无矩形标签底框。
- [x] 纯几何测试覆盖米制面积与多边形中心计算。
- [x] 真实测绘页核对已保存面积的中心位置和标签可读性。

final result: passed

## 2026-08-05 浏览器批注：事件关键帧直达测绘

### Findings and fixes

- 真实事件详情底部新增“事件测绘”，与驳回、技术确认并列；仅当前事件存在可复用原始关键帧时展示。
- 点击后调用幂等事件测绘接口，保留事件、Mission、Pipeline、SourceProfile 和原证据谱系，随后直接进入返回任务的点线面量算页。
- 原始 JPEG 复用内容地址，派生 BEV 使用同源同步遥测生成；无证据、遥测或米制变换时服务端拒绝创建空测绘任务。

### Verification

- [x] 真实事件 `e1dcc7234534e764416f7f71e58a6d65053fc33e` 创建并复用任务 `SVY-20260805-9299D2`。
- [x] 浏览器点击“事件测绘”进入 `/survey/SVY-20260805-9299D2/measure`，当前关键帧 BEV 可见且测量变换为 available。
- [x] 同一幂等键重复请求返回同一任务，浏览器控制台 0 error / 0 warning。

final result: passed

## 2026-08-05 实时目标即时投放与质量卡顺序

- 实时策略取消 ByteTrack 新建轨迹的一帧确认等待及业务层 2 秒/5 点成熟等待；达到检测/关联阈值即作为目标投放，历史完成轨迹的归档质量门槛保持不变。
- Monitoring 的地图图例默认只显示“目标轨迹”；仅在收到旧消息显式候选时显示“兼容候选轨迹”。
- 左侧态势面板按操作优先级先展示实时轨迹、关键指标和图表，“巡航与悬停融合质量状态”移动到面板最底部。
- 工作台无人机源标记改为 44px 完整占位容器，32px 图标面与数量角标都位于容器内，并提升到独立 300/320 图层，避免密集路口标记遮挡或裁切无人机图标。
- 无人机源覆盖不再依赖服务器典型矩阵是否包含同一 `inter_id`；小清河北路、海右路、礼士路、崇华路等具备已验证项目坐标的路口分别落图并按路口聚合，缺坐标源继续不展示，避免伪造位置。
- 测绘报告证据图读取失败时不再永久显示加载态，而是展示真实错误和“重试图片”；Replay V2 与其他本机 profile 统一读取 `.runtime/survey` canonical 证据目录。

## 2026-08-05 Replay V2 实时轨迹投放恢复

### Findings and fixes

- P1：Replay V2 MetricStore 拒绝携带实时尾迹的 Stats，导致 Kafka 记录隔离，浏览器一直显示 0 TRACKS。修复为只忽略尾迹持久化、保留 WebSocket 原样投放。
- P1：当前经十路源处于超出正式位姿包线的 171m 高度，ENU/GCJ-02 按质量门禁为空；右侧卡片原先只能显示空地图。修复为有 GCJ-02 时投放高德地图，仅有 `trajectory_px` 时自动切换独立像素坐标画布，不猜测地理坐标。
- P1：成熟活动轨迹因地理/路网质量降级被错误绘制成候选虚线。修复为活动轨迹实线、真正候选轨迹橙色虚线，并在画布内加入线型图例。
- P1：Replay V2 缕合适配器缺少实时监控 Stats/Conflict 查询，导致趋势和历史冲突请求 500；已补齐只读 V2 数据面的查询契约。

### Verification

- [x] PostgreSQL Replay V2 集成测试通过，实时尾迹不落历史样本，Stats/Conflict 查询返回 V2 谱系。
- [x] Kafka/Runtime/管道相关测试 61 passed；像素轨迹画布组件 11 passed。
- [x] Vite production build。
- [x] 浏览器使用当前 Pipeline 验证实时轨迹数量与坐标模式文案。

final result: passed

## 2026-08-05 浏览器批注：轨迹研判调整为智能研判首个页签

### Change

- 顶部“智能研判”二级导航顺序调整为“轨迹研判 / AI 事件中心”。
- 一级业务域点击“智能研判”时沿用首项路由，因此默认进入 `/gis`。
- 删除 AI 事件中心顶部“本机演示分析快照”面板，事件筛选与 road9 真实事件列表直接承接页面首屏；已有 `snapshot_id` 深链事后分析能力保留。
- 首页 KPI 删除“机非冲突”“治理提升”两项固定演示值，只保留服务器返回的态势路口、过饱和路口和拥堵路段。
- 事件台账固定读取最近 150 条；本轮 road9 实际合计 144 条，未发生超限删除。
- `replay_v2` 恢复无人机、视频/遥测源、飞行计划和 Mission 的完整运行能力；旧只读门禁删除，任务调度、Pipeline 启停和实时派发启用，数据面仍隔离在 `uav_replay_v2_*`。
- 页面路由、权限和事件业务内容保持不变。

### Verification

- [x] `RouterApp.test.jsx` 导航顺序与 active 状态。
- [x] Console2 聚焦回归 61 项通过；production build 通过。
- [x] 1357 × 912 浏览器刷新后，“轨迹研判”位于第一项且保持 active。
- [x] AI 事件中心刷新后不再出现本机快照面板，显示 144 / 144 条且无不可用告警。
- [x] 首页 KPI 仅保留 3 项服务器态势指标；地图实测绘制 95 个路口分级图标和 1 个无人机图标。
- [x] 无人机页刷新后显示 6 架无人机、14 个数据源、1 个飞行计划和 73 条执行记录，回放源下拉框恢复。
