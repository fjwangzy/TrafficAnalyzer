...
import { ArrowCounterClockwise, CloudArrowUp, Database, DownloadSimple, Gauge, GitDiff, Link as LinkIcon, LockKey, Pulse, ShieldCheck, Stack, Warning, X } from '@phosphor-icons/react'
...
export function SystemPage() {
  ...
  return <AppShell pageTitle='系统与身份'>
    ...
    {tab === 'identity' && <><QualityNotice tone='info' title='只读身份源'>当前接口仅提供真实用户同步结果；Console2 不提供模拟邀请、编辑或删除。</QualityNotice><Panel title='身份同步结果'>{usersQuery.isLoading ? <div className='live-state'>正在加载用户…</div> : platformUsers.length ? <DataTable rows={platformUsers} columns={[{ key: 'status', label: '状态', render: (value) => <StatusBadge value={value} /> }, { key: 'id', label: '用户 ID' }, { key: 'name', label: '用户名' }, { key: 'account', label: '邮箱' }, { key: 'role', label: '平台角色' }, { key: 'scope', label: '数据范围' }, { key: 'syncedAt', label: '创建时间' }]} /> : <div className='live-state empty'>暂无用户数据</div>}</Panel></>}
  </AppShell>
}
