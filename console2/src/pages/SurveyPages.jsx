...
export function SurveyCapturePage() {
  ...
  const importAssets = async () => {
    setBusy(true)
    try {
      await platformApi.importSurveyBatch(task.id, { video_asset: 'inter_xqh/DJI_20260403142902_0001_V小清河北路与水屯路路口.mp4', telemetry_asset: 'inter_xqh/telemetry.srt' }, requestKey('survey-import'))
      await Promise.all([loadBatches(), reloadTask()]); notify(dispatch, 'info', '真实 MP4 + SRT 已进入后台处理队列')
...
export function SurveyMeasurePage() {
  ...
  const reload = useCallback(async () => {
    if (!task) return
    const [frameRows, measurementRows] = await Promise.all([platformApi.surveyFrames(task.id, task.selected_batch_id), platformApi.surveyMeasurements(task.id)])
    setFrames(frameRows); setMeasurements(measurementRows); setFrameId((current) => current || frameRows.find((item) => item.has_metric_transform)?.id || '')
  }, [task])
...
export function SurveyReviewPage() {
  ...
  return <SurveyShell task={task} step={3} title='技术复核' description='逐项对照原始帧、BEV 示意、量算结果和质量边界；通过只表示技术复核，不等同法定事故认定。' actions={<><button className='danger-button' disabled={task.status !== 'pending_review'} onClick={() => setReasonOpen(true)}><X size={15} /> 退回修订/补拍</button><button className='primary-button' disabled={task.status !== 'pending_review'} onClick={() => act('approve_review')}><Check size={15} /> 技术复核通过</button></>}>
    <div className='review-layout'><Panel title='原始帧与正射图对照'><div className='compare-panes'>{frame ? <><div><span>原始帧 #{frame.frame_number}</span><EvidenceImage url={frame.image_url} alt='真实原始关键帧' /></div><div><span>BEV 量算底图</span><EvidenceImage url={frame.bev_url} alt='真实 BEV 关键帧' /></div></> : <div className='survey-empty'>未找到证据帧</div>}</div></Panel><div className='review-side'><Panel title='复核清单'>{['任务与位置一致', '原始材料引用完整', '像素至 ENU 坐标链可追溯', `量算对象 ${measurements.length} 项`, '人工编辑版本完整', '误差阈值未冻结已明确标识'].map((item) => <label className='check-row' key={item}><input type='checkbox' defaultChecked /><CheckCircle size={16} weight='fill' />{item}</label>)}</Panel><Panel title='量算结果'>{measurements.map((item) => <InfoRow key={item.id} label={item.category || item.id} value={item.display_value} badge={item.quality_status} />)}</Panel></div></div>
