...
- 遥测单应性的 Nadir 与完整透视分支统一按 DJI 航向约定解释 `gimbal_yaw`：从北顺时针；转换到 ENU 数学旋转时完整透视分支使用相反符号，跨横滚分支阈值不得产生航向跳变。
