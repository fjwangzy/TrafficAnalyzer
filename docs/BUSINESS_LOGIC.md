...
8. 场景标注只能关联已持久化关键帧，车辆、痕迹、散落物和其他对象的创建/修改/删除保留 revision 与统一审计；Kafka 悬停自动生成车道任务默认关闭，管理员仍可从已持久化真实关键帧显式形成审计任务，但确认结果不再进入视频源启动参数。所有 Pipeline 固定以 `ROADS_JSON=""` 运行，当前任务统一标记 `invalidated`、视觉绑定统一 `retired`。
