...
## 2026-08-22 易出行无人机交通治理业务汇报改版

- [x] 保留原版 `docs/generated/无人机交通治理应用整体情况.pptx`，新增 11 页领导决策与交管业务版 `docs/generated/易出行无人机交通治理业务成效与试点汇报.pptx`；沿用原母版、字体、页码、品牌标识和白底蓝橙体系。
- [x] 按易出行“一图一池三智”重构叙事：无人机负责机动感知、风险证据和同口径复测，结果进入治理闭环与统一事实池；主平台、交通智能体和人工审批链路负责诊断、策略及控制执行。
- [x] 数字口径分为“已形成／试点目标／待业务验证”：已形成仅覆盖真视频、真库、可控回放、Mission 谱系和可复核证据；`≤30 秒`、`≤15 分钟`、冲突 Precision `≥85%` 均明确标记为目标，不能表述为既有交通治理成效。
- [x] 第 3 页使用无文字、无第三方标识、无数值结论的 AI 概念插图，仅表达无人机加入易出行后的业务关系；逐页演讲者备注均包含 `[Sources]`，引用易出行 Wiki、无人机 PRD 与仓库内真实验证材料。
- [x] 2026-08-25 按“标题页不计入汇报页号”整理第 4、5 页：第 4 页突出“路口状态实时感知—优化方案自动匹配—信号配时自动下发—运行状态反馈闭环”，第 5 页总结世界坐标时空轨迹的可回放、可计算、可复盘和可仿真价值，并保留主平台控制责任及坐标/路网质量门禁。
- [x] 2026-08-25 针对“打开后未见变化”重新核对磁盘文件与页码映射，并将汇报第 4、5 页二次改为显著蓝橙流程块；原路径重新写入，同时新增 `docs/generated/易出行无人机交通治理业务成效与试点汇报-第4-5页显著改版.pptx` 规避 PowerPoint 旧窗口或缓存副本。
- [ ] 正式业务成效仍待外部门禁：确认 1—2 个试点点位，冻结同路口、同星期、同五分钟时段和可比飞行条件；开放权威路网、主平台/数仓接口与策略回执；完成生产安全、模型质量及多路口业务验收。

## 2026-08-10 XQH 典型高夹角 TCC 当前检测器恢复

- [x] 定位目标漏检唯一门禁为当前 ENU 夹角 `160.8° > 150°`，保留标准角度口径并新增严格 `150°~170°` 左转路径交点分支。
- [x] 以视觉证据收紧高夹角门控：TTC/PET、避险行为、明确左转弧线缺一不可；近对向直行以 `high_angle_scene_ambiguous` 拒绝，CPA 继续关闭。
- [x] 修复 Replay V2 Kafka zstd 双端依赖和临时依赖遮蔽，统一单一 8000 MPS 运行实例；缺少编码依赖时启动前失败关闭。
- [x] 首帧发布 `running` Mission 事实，使冲突在播放过程中即可被 Event Center 查询，不再等待 EOF/人工停止。
- [x] 真实 XQH Mission `MSN-DD60921B64E2` 在运行中输出目标事件 `ddb7ca9e...`：T+33.333s、critical、TTC 2.16s、PET 0.03s、两张 3840×2160 证据；浏览器确认图片及 TCC 判定依据可见。
...
- [x] 首页原右侧治理摘要（重点路口态势、重点事件、治理复盘）整体移入左侧上下文区域；点击地图无人机/视频源后整块切换为无人机飞行与实时指标面板，点击返回按钮、普通路口/路段或地图空白处均恢复完整治理摘要。右侧栏及收起按钮删除，两组内容不再同时挤占地图。
- [x] 首页选择运行中无人机后进入车辆数字孪生：严格消费当前 Pipeline 的成熟 `active_trajectories`，增量动画展示 GCJ-02 小车与短尾迹；按 `lane_verified` Lane、可信 Link、空间轨迹、像素覆盖仿真和无轨迹五级降级，停止线/渠化区域独立可空。像素层仅在位置/高度/近垂直云台/相机视场齐备时进入高德卫星影像的近似覆盖框，按云台偏航自适应旋转地图并按逆缩放契约计算视场；明确不构成 GCP、Lane/Link 或世界坐标事实。开发态 `?twinPreview=1` 提供隔离 Mock；真实经十路 stride 3 Mission 已完成一小时可控回放浏览器验收与自然窗口收口。
- [x] 修复 XQH 数字孪生相机抖动与缩放失配：正式世界图层固定北向 3D，短暂降为像素样本也不旋转；首次取景只按当前车辆位置（纯像素按覆盖框），不再把约 1km 的 64 Lane 全量范围和历史尾迹一起缩放，同一任务轮询不重复 `fitView`。真实 XQH 页面跨刷新周期保持同一 AMap 实例、50m 比例尺和 0 console error。
- [x] XQH 首页取景以选中无人机为锚点，将其从整图几何中心向右偏移 20%（屏幕横向 70%），与左侧详情面板之外的可视区域中心对齐；复位沿用相同车辆缩放和无人机偏移，不回到城市级视图。
- [x] 数字孪生模式移除无人机地图 hover 详情卡，飞行详情统一留在左侧面板；选中无人机 Marker 提升到 `zIndex=1000`，高于其他无人机 `680` 和车辆 `620`，退出仿真后恢复 hover，切换过程不重建地图。真实页面在 83 辆车下验证 hover 后卡片节点仍为 0。
- [x] 首页无人机飞行画面支持双击进入对应实时监控大屏，复用当前 `intersection_id + source_profile_id`；修复监控页注册源加载期间把请求深链抢先覆盖成 `SRC-DEMO-XQH` 的竞态。真实经十路复验保持 `INT_MP4728_JINGSHI_CORRIDOR + SRC-MP4729-JS-0729-3MS`，并选中对应 3m/s 视频源与检测器画面。
- [ ] 经十路完整回放封存门禁：一小时 stride 3 Mission 只推进到源时间 `226.833s / 579s`，Replay V2 仅落库 1321 条指标样本，诊断仍为 `incomplete` 且无 Track Event/Point。需以足够窗口或更高 stride 跑到真实 EOF，并补齐当前 `ReplayV2MetricStoreAdapter.query_tracks`（通用 `/trajectories/{intersection_id}` 当前返回 500）后，才能声明 sealed Mission 与完整轨迹回放通过。
...
| T-507 | mp4820 巡航 TCC 真实性验证 | ✅ | 按原节点 seam 完成有界遥测插值、视觉位姿残差 fail-closed、稳健速度/方向、当前帧双参与者/边界净距、跨类包含拒绝、正式 track ID 冷却和精确事件参与者修复，未替换 YOLO/ByteTrack/Kafka/road9。聚焦回归 115、Platform 316/6 skipped/10 subtests、核心 30、XQH 55/0/1、评审包 2 和 Console build 通过。EW/WE 均原生 MPS 自然 EOF：1,929/1,347 stats、6,362/7,536 唯一完成轨迹、7/13 个新 TCC；用户三个历史时刻无正式事件。20 个新事件的 managed 原图、检测图和 10 秒片段全部人工确认，`confirmed=20 / false_positive=0 / uncertain=0`。WE 末段 Docker 满盘造成 road9 recovery；在保留主机 Kafka 副本后仅移除停用 Kafka 容器及其 12.9GB 卷，恢复后 road9 按唯一消息精确对账 1,347/7,536/13/3,063，原故障快照保留。EW/WE TCC eligible 分别为 79.47%/53.23%，均未达 90% 严格门槛，不降低门槛换通过；生产精度、召回率、IDF1/HOTA、位置 RMSE 和速度 MAE仍为 `not_evaluated`。证据见 `docs/2026-08-21-mp4820-tcc-remediation-validation.md`。 |
| T-508 | MP4820 同轨迹对追加误报与事件标题真实性 | ✅ | 精确驳回 `22cf739c...` / `f29dd04d...` 为 revision 2；Event Center 仅把 confirmed 展示为已确认事件，pending/rejected/Replay V2 均保持候选语义。原生 MPS 双源新管道自然 EOF、返回码 0、非法 TCC 0，EW 排空后 `3153/6362/7/4257`、WE `3217/7536/13/3836` 与 road9 精确一致，均未再输出轨迹 3752/3059。Docker 虚拟盘无损扩容至 128GB，runner 可按 next offset 恢复 Kafka 采集。严格覆盖率 75.61%/51.73% 仍未过 90%，不降低门槛。 |
| T-509 | 实时监测方向流量与重点场景策略面板 | ✅ | 在既有监控信息架构内保留最左全局导航、第二层左侧实时态势、中央视频和右侧分析；左右业务侧栏继续支持边缘收缩、悬停展开与固定。物理流向的流量、均速、排队和饱和度统一放入左侧态势指标区；右侧仅保留 BEV、六类策略和近期事件。真实源依次读取显式 Movement、可解析 AutoLane 观测、活动 GCJ-02 轨迹方位推断；无事实时显示等待数据，只有显式固定演示源允许演示样例。活动轨迹总数改读未截断的 `active_tracks`；固定演示统计、事件和趋势也不再回退到已登记真实源。无 `lane_verified` 时 AutoLane 的米制排队按物理方向取最大值降级展示，并标记“降级观测 / 方向估算”；饱和度继续显示“未标定”。检测管道保证只有有效 H 换算值进入 `queue_length_m`，像素回退改写 `queue_length_px + queue_length_unit=px`，Console 不把像素当米。按最终批注删除底部实时数据时间轴及暂停/冻结控制，左右面板底部从 88px 扩展到 14px，中央视频直接贴底。历史统计适配器补齐 `pipeline_id` 过滤，修复 `/intersections/{id}/stats` 500。xqh Mission `MSN-E8FCEF371A8A` / Pipeline `pipe-8aa7ee61` 最终同帧核验：页面 `257 辆 / 247 条 / 22.5km/h` 与 Kafka message `5b9d520d-0fd1-42b2-ade9-ad56181c13b1` 完全一致；四个聚合 Movement `18.7 / 16.3 / 12.0 / 10.2 辆/分钟` 逐项回算一致，方向最长排队显示 91m。六类策略均已扩为“适用场景—业务目标—联合判据—处置链—控制边界—退出与复盘”；黄闪保留交警审批，可变车道强制先清空，溢流按“以出量进”。真实源证据不齐时显示“不触发”，固定演示明确标为非实时；当前没有信控下发接口，页面只生成建议并要求交警或信控平台确认。 |
| T-510 | 实时监测业务评审整改与 xqh 验收 | ✅ | Console2 新增实时源健康/延迟/过期门禁，将开发回放明确排除在生产实时性验收之外；TCC 按 ID、源、Pipeline、参与轨迹对和 60 秒窗口聚合，风险口径区分触发、事件、已复核和待复核。事件详情读取权威台账并加载原始帧/检测帧，技术复核使用 revision 和管理员权限，Replay V2 历史事实保持只读；本地原始帧可建立证据复核任务，原“事故测绘”操作改为“事件治理台账”。六场景按证据完备度排序，缺少道路、信号、设施或外部接口时不形成在线建议；选中事件突出参与轨迹，其余轨迹降透明度；工程诊断与业务态势分层并提高目标视口可读性。xqh 浏览器验收为开发回放、策略门禁阻断、13 次触发聚合为 12 起待复核事件、原始/检测证据可见、六场景均不形成在线建议，Console 0 warning/error。定向 `82/82`、Console2 全量 `251/251` 和 production build 通过。生产验收仍需真实无人机在线流及交警/信控外部接口现场联调，不以回放代替。详见 `docs/2026-08-24-xqh-monitoring-business-review.md`。 |
| T-511 | 场景研判标签精简 | ✅ | 按浏览器批注移除“场景研判”标题右侧“一次仅研判 1 项”，并隐藏六类策略卡片及当前研判中的“开发测试”标签；底层开发回放状态和“不形成在线建议”控制边界保持不变。 |
| T-512 | 崇华路晚高峰检测启动恢复 | ✅ | 诊断并清除 `replay_v2` 与标准 `live` Platform 同占 `8000` 导致的 launchd 重启环，恢复单一标准 `live` 实例；修复 SRT 明确给出 `zoom_factor=None` 时 Homography 计算抛出 `TypeError` 的问题，缺失镜头倍率现在只关闭世界坐标/TCC 正式门禁，不再终止检测和图像跟踪。真实来源 `SRC-MP4NEW2-CH-0715-PM` 已由 Mission `MSN-6747BBF1661D` / Pipeline `pipe-cc7f9b97` 启动并保持运行。 |
| T-513 | 高度与默认镜头门禁校正 | ✅ | 删除 `FlightMotionClassifier` 的 AGL 60–150m 固定姿态/TCC 门禁，避免把来源中的海拔或其他高度语义误判为真实 AGL；镜头规则改为全局默认标准广角 1x，并保留来源级 `auto_from_telemetry` 特殊覆盖。高度来源、当前帧矩阵、飞行姿态和视觉位姿等独立世界/TCC 门禁不变。 |
| T-514 | 崇华路 BEV 航向与 TCC 证据闭环修复 | ✅ | 修正完整透视单应性分支对 DJI 顺时针北向 yaw 的符号解释，消除横滚跨 0.5° 时 ENU 航向突跳；Event Center 在分页前按路口、SourceProfile、Mission 筛选冲突，并允许实时 `source_message_id` 解析到持久事件主键后复核；监测页证据缩略图支持点击大图查看。新增投影连续性、事件筛选/别名和证据大图回归。 |
...
| T-506 | XQH TCC 聚焦画面与原事件证据重建 | ✅ | `ShowNode` 对活跃 TCC 帧先过滤非冲突目标和常规统计/道路叠加，只保留双方框、标签和实线历史轨迹；两条预测轨迹以虚线从双方当前端点收敛到 ENU 预测冲突点，并分别继承对应对象检测框和历史轨迹的颜色，以红黄爆点表达 💥。TTC 徽章在爆点周围动态选位，避开双方框、标签及历史/预测轨迹。渲染/证据发布聚焦回归 `31 passed`。真实重放在 `T+33.333s` 再次命中 `1802/1770 / TTC=2.16s / PET=0.03s`，新旧原始帧 SHA 同为 `dae77839...3e9`；事件 `ddb7ca9e...` 保持 ID、Mission、业务事实和原图不变，detector evidence 原 ID 换版为 SHA `a64b7a20...d107`、EvidencePackage version 4。临时重放的 2 条重复冲突引用已移除，受管对象未物理删除；Platform 单一 8000 实例健康且无活动 Pipeline。 |
