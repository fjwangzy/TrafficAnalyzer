...
- [x] 增加版本化 SourceGeoRegistration、旧 verified VisualRegistration 幂等回填、Mission/Pipeline 独立配准固定、地图可选及版本/anchor 不兼容降级；Alembic head `20260728_0020` 已在 road9 应用且未修改历史业务事实。
...
| T-501 | AGL 三档 imgsz、能力感知运行档与性能/TCC 口径 | ✅ | 已实现共享 `AdaptiveImageSizePolicy`（640/960/1280、5点中位数、5m滞回、5帧稳定、2秒缺失回退），接入新旧检测节点；Mission/FlightPlan 按 verified SourceGeoRegistration 自动选择 cruise/legacy 并固化原因；Stats 分开 YOLO 与整帧耗时并扩充 TCC 漏斗。xqh 自然 EOF 27/27，1142 帧全为 960、零切档。按授权只清空历史 `uav_track_points/uav_track_events/uav_conflict_events/uav_conflict_reviews` 后，崇华路 v3 原生 MPS 自然 EOF：1299 stats、6302 完成轨迹、56 条严格 `path_intersection` TCC、`invalid_tcc_events=[]`，88 个唯一受管 JPEG 的 hash/size 全通过；积压归零后 Kafka/road9 精确一致为 `1299/6302/56/2407`。对齐视频的 AGL 实际为 156.35–178.15m，因首帧初始化 1280 且下切阈值为 `<152m`，全程合法保持 1280；原计划引用 104–222m 是整个遥测文件而非视频对齐窗口，不能作为三档切换证据。1280 档 YOLO p50/p95/max 为 371.4/1554.0/3990.6ms；xqh 960 p50=165.8ms，均未过 100ms 性能门。Platform 慢消息消费改为单条 poll、30 分钟处理窗口，避免密集 Stats 写 Timescale 时超时再均衡；功能实现与验收执行完成，但生产性能仍未达标，精度继续 `not_evaluated`。 |
