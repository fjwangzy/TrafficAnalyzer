...
| S3 | 事故测绘 | 测绘任务、量算、误差、示意数据及证据管理 | 事故民警、测绘、法制 | 详细评审草案；量算与法制口径待冻结 | [S3 事故测绘 PRD](uav-traffic-ai-prd/S3-accident-survey-prd.md) |
...
分册可只展示负责人和当前状态，以避免七册重复维护排期；计划关闭时间和关闭依据由第 14 章总台账集中管理并作为唯一进度真源。总台账聚合同主题事项时，关闭任一分册项前必须记录该项自己的结论和证据；若计划时间或关闭依据不同，应拆分总项后再关闭。
...
3. **最后冻结 S7**：汇总各分册指标，形成统一数据集、试点、性能、上线和运维验收方案。
4. 分册进入开发前，至少完成：范围确认、主流程、输入输出、核心规则、异常降级、验收指标、待补充责任人七项。
...
目标数据域至少覆盖以下范围。前三行是一期 canonical 持久化基线；测绘、执法、证据、路网及既有平台业务行是 PRD 级规划表目录，只有在数据库分册给出不重复的实体映射并经迁移/DDL 评审冻结后才可建表，不得把同一实体同时落入通用表和场景表形成双真源。
...
| 任务 `MISSIONS` | 任务业务状态持久化 | `uav_missions` |
...
| 视频 `_STREAMS` 运行注册表 | 默认继续作为易失运行态；只有明确需要跨重启配置时才建表 | `uav_video_sources` 为候选，当前 `【待确认】` |
...
> 📌 **状态标注**：✅ 前端已实现 | 🔧 已有基础需增强 | 🆕 待开发

**导航结构：指挥大屏**

| 序号 | 页面/功能 | 路由 | 状态 | 说明 |
| --- | --- | --- | --- | --- |
| 1 | Dashboard 总览 | `/` | ✅ 已有 | KPI卡片（活跃路口/总流量/拥堵指数/异常事件）、24h流量趋势、系统状态（无人机在线/管道活跃/延迟）、路口列表、最近告警 |
| 2 | 实时监测 | `/monitoring` | ✅ 已有 | 双面板（MJPEG视频流 + BEV轨迹地图）、车道级指标（流量/车头时距/排队/速度）、转向分布、30min流量趋势、拥堵指数仪表盘、告警时间线、管道启停控制 |
| 3 | GIS 总览 | `/gis` | ✅ 已有 | 全部路口地图标注、网格化路口标记、活跃路口计数、路口详情面板 |

**导航结构：业务操作**

| 序号 | 页面/功能 | 路由 | 状态 | 说明 |
| --- | --- | --- | --- | --- |
| 4 | 视频分析 | `/video` | ✅ 已有 | 多摄像头选择（3路）、MJPEG流显示、实时统计叠加（FPS/推理耗时/车辆数）、冲突事件检测、单摄像头管道管理 |
| 5 | 标定中心 | `/calibration` | ✅ 已有 | 单应性标定记录、车道标注任务、车道多边形编辑器、标定覆盖度可视化、质量指示器（OK/Interp/Degraded/Missing） |
| 6 | AI事件中心 | `/alerts` | ✅ 已有 | 实时 AI 事件表、详情、技术复核和推送日志；本地风险等级不直接等同主平台警情等级，指挥处置由智慧交通主平台完成 |
| 7 | 报告中心 | `/reports` | ✅ 已有 | 时段选择（24h/7d/30d）、路口选择、交通统计聚合、趋势分析图表 |

**导航结构：管理**

| 序号 | 页面/功能 | 路由 | 状态 | 说明 |
| --- | --- | --- | --- | --- |
| 8 | 无人机管理 | `/drones` | ✅ 已有 | 无人机列表（在线计数）、遥测详情（电池/位置/高度/速度）、任务列表、飞行轨迹可视化、3秒遥测刷新 |
| 9 | 系统管理 | `/admin` | ✅ 已有 | 系统健康状态、GPU利用率、Kafka Topic/消费者、ML模型状态、用户列表、JSON数据查看器 |
| 10 | 用户管理 | `/users` | ✅ 已有 | 只读用户表（ID/用户名/邮箱/角色/状态/创建时间） |

**导航结构：AI 模型新增功能（🆕 待开发）**

| 序号 | 页面/功能 | 路由（建议） | 状态 | 说明 |
| --- | --- | --- | --- | --- |
| 11 | 风险热区 | `/risk-hotspots` | 🆕 待开发 | 事件空间聚类可视化、GCJ02地图标注、热区半径/事件数/最高风险等级 |
| 12 | 执法区域配置 | `/enforcement/zones` | 🆕 待开发 | 电子围栏绘制（GCJ02多边形）、区域类型（限速/限行/禁停/管控）、关联规则配置 |
| 13 | 执法事件管理 | `/enforcement/events` | 🆕 待开发 | 执法事件列表（超速/货车限行/违停/滞留/占道）、证据链查看、复核确认/驳回、导出 |
| 14 | 货车监控 | `/enforcement/trucks` | 🆕 待开发 | 货车实时位置、限行判定、违规事件触发 |
| 15 | 测绘任务管理 | `/survey` | 🆕 待开发 | 测绘任务创建/列表/状态、量算结果查看、证据帧管理 |
| 16 | 测绘报告 | `/survey/{id}/report` | 🆕 待开发 | 事故车辆位置标注、散落物分布、刹车痕迹、平面示意图、误差报告、导出 |
| 17 | 规则配置 | `/admin/rules` | 🆕 待开发 | 拥堵/冲突/执法规则JSON配置、启用/停用、热更新 |
| 18 | 证据链管理 | `/admin/evidence` | 🆕 待开发 | 视频帧+轨迹+位置+规则+时间戳 查看、保留策略管理 |

**增强现有模块（🔧 需增强）**

| 序号 | 增强内容 | 目标模块 | 状态 | 说明 |
| --- | --- | --- | --- | --- |
| 19 | 多因子拥堵指数（0-10分） | 实时监测 `/monitoring` | 🔧 增强 | 车辆密度分+排队长度占比分+低速比例分 |
| 20 | 转向流量增强 | 实时监测 `/monitoring` | 🔧 增强 | GCJ02坐标轨迹+进口道×转向交叉统计 |
| 21 | 换道事件显示 | 视频分析 `/video` | 🔧 增强 | 换道事件标注+几何/运动双重验证结果 |
| 22 | TTC/PET双指标 | 视频分析 `/video` | 🔧 增强 | 碰撞时间和后车侵入时间实时显示 |
| 23 | 告警类型扩展 | 告警中心 `/alerts` | 🔧 增强 | 新增换道/超速/违停/货车限行等告警类型 |
...
> 以下模块已在 traffic-fly-console 前端实现，PRD 记录其现有功能和与 AI 模型的集成增强点。

##### 10.2.0.1 Dashboard 总览（`/`）

**现有功能：**
- **KPI 卡片**：活跃路口数、总流量（辆/分钟）、拥堵指数、异常事件数
- **24h 流量趋势图**：折线图，WebSocket 实时更新
...
**与 AI 模型集成增强点：**
- 🔧 KPI 卡片增加多因子拥堵指数（0-10 分）
- 🔧 新增风险事件计数（换道/冲突/执法）

##### 10.2.0.2 实时监测（`/monitoring`）

**现有功能：**
- **双面板布局**：左侧 MJPEG 视频流 + 右侧 BEV 轨迹地图（OpenLayers）
...
| 角色名称 | 角色说明 | 典型人群 | 数据范围 |
| --- | --- | --- | --- |
| 系统管理员 | 技术配置、规则候选配置和本地运维账号管理 | IT 运维人员 | 按统一身份/主平台授权的数据范围；紧急运维权限单独审计 |
...
> 导航结构与前端 traffic-fly-console 对齐

**指挥大屏**

| 序号 | 页面 | 页面元素 | 管理员 | 指挥员 | 执法人员 | 事故民警 | 分析员 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | Dashboard (`/`) | — | ✓ | ✓ | ✓ | ✓ | ✓ |
| 2 | 实时监测 (`/monitoring`) | — | ✓ | ✓ | ✓ | ✓ | ✓ |
| 3 | 实时监测 | "管道启停" | ✓ | — | — | — | — |
...
| 序号 | 页面 | 页面元素 | 管理员 | 指挥员 | 执法人员 | 事故民警 | 分析员 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 16 | 风险热区 (`/risk-hotspots`) | — | ✓ | ✓ | ✓ | — | ✓ |
| 17 | 执法区域 (`/enforcement/zones`) | "新建/编辑" | ✓ | — | ✓ | — | — |
| 18 | 执法事件 (`/enforcement/events`) | — | ✓ | — | ✓ | — | ✓ |
| 19 | 执法事件 | "确认/驳回/导出" | ✓ | — | ✓ | — | — |
| 20 | 货车监控 (`/enforcement/trucks`) | — | ✓ | — | ✓ | — | — |
| 21 | 测绘任务 (`/survey`) | — | ✓ | — | — | ✓ | — |
| 22 | 测绘任务 | "创建任务/导出" | ✓ | — | — | ✓ | — |
| 23 | 规则配置 (`/admin/rules`) | — | ✓ | — | — | — | — |
| 24 | 证据链 (`/admin/evidence`) | — | ✓ | — | ✓ | — | — |
...
- **兼容页面约束**：现有 `/users` 页面若保留，应调整为统一身份同步/只读查看或仅管理本地运维账号；最终身份集成方式由 S6 冻结。
...
> 上述条目已覆盖 S1-TBD-001~013、S2-TBD-001~012、S3-TBD-001~012、S4-TBD-001~011、S5-TBD-001~014、S6-TBD-001~010 和 S7-TBD-001~013。一个总项可聚合多个同主题分册项，但关闭时必须逐个记录结果；分册与总台账严重度不一致时取更严格状态。轨迹预测合同符合性只有在 P-TBD-001 取得书面依据后才能关闭。
