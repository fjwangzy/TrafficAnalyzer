...
| 2026-06-29 | 前端功能融合：将 traffic-fly-console 已有 10 个功能模块详细描述融入 PRD，标注已有/增强/待开发状态 | 前端现状对齐 | [TODO] | [TODO] | v1.2 |
...
现有系统已在多个路口完成端到端验证（inter_xqh 视频 4K@30fps，2026-07-02: 56 PASS / 0 FAIL / 0 WARN），具备从视频采集到结构化数据输出的基础闭环能力。
...
| 6 | 告警中心 | `/alerts` | ✅ 已有 | 实时告警表（筛选：全部/未处理/已确认/优先级P1-P3）、告警详情面板（快照/VLM摘要/推送日志）、确认工作流、事件详情（截图/视频片段/轨迹）、Webhook管理（URL/Token/严重级别/频率限制/自定义Payload） |
...
| --- | --- | --- | --- | --- |
| 11 | 风险热区 | `/risk-hotspots` | 🆕 待开发 | 事件空间聚类可视化、GCG02地图标注、热区半径/事件数/最高风险等级 |
...
| 20 | 转向流量增强 | 实时监测 `/monitoring` | 🔧 增强 | GCG02坐标轨迹+进口道×转向交叉统计 |
...
- **最近告警**：最新告警条目，WebSocket `alerts` 频道推送

**WebSocket 订阅**：`alerts`（告警推送）、`system`（系统指标）
...
**WebSocket 订阅**：`intersection:{id}`（stats + track_complete）
...
**WebSocket 订阅**：`intersection:{id}`（stats + conflict）
...
- 🔧 高风险事件（TTC<1.5s 或 PET<1.0s）自动标记 P1 优先级
