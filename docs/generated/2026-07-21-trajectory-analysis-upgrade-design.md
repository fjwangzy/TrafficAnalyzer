...
> 状态：产品与交互设计已冻结，代码、迁移、四路口真实数据与浏览器同视口视觉验收均已完成；业务阈值、权威路网和生产容量仍遵循 S1/S5/S7 的外部门禁
...
2. 右栏默认显示“流向排名”，先按证据完整性、再按车辆数排列，并同时给出占比、均速和冲突数；可切换车辆数、均速或冲突排序。
...
- 流向主键优先使用 `entry_link_id + exit_link_id`；出口不可可靠匹配时，先使用明确标注为 `trajectory_quadrant_inferred` 的 ENU 首末方位，再降级为 `entry_link_id + turn_behavior`。
- 展示名称优先使用 RoadContext 的道路/Link 名称，例如“水屯路南进口 → 小清河北路东出口”。只有本地道路编号时显示“道路 3 → 左转”；使用轨迹方位时必须显示“轨迹方位推断”，不得冒充权威路网匹配。
- `unknown`、`unmapped` 独立成组并显示质量标记，不静默并入其他流向。
- 排名默认按证据完整性展示有效流向，同一证据等级按 `vehicle_count` 降序；允许切换“车辆数”“均速”和“冲突”，切换只改变排序，不改变当前筛选集合。
- 车辆数按当前查询总窗口内的唯一完成轨迹去重，不按轨迹点计数。占比的分母为同一筛选条件下可进入流向聚合的轨迹数。
- 均速只聚合有效米制速度；无有效样本显示 `—`，不以 0 代替。冲突数只统计可安全关联到该流向轨迹的真实冲突事件。

### 3.3 时间片语义

- 默认查询所选筛选条件下“最新一条可回放轨迹”所在的 30 分钟数据段，回放时间片长度和步长均为 10 秒；避免离线历史被当前墙钟时间切空。
- 地图显示与时间片相交的轨迹片段，不显示查询窗口内所有完整轨迹；ENU 与 GCJ-02 必须使用同一组时间索引和抽样索引裁剪。
- 时间轴柱高表示每个桶内不同活跃轨迹数；同一轨迹在一个桶内只计一次。
- 播放速度提供 `0.5× / 1× / 2× / 4×`，默认 `1×`；播放到末尾后停止，不自动跳回实时。
- 自动播放只推进到具有活跃轨迹或冲突的时间片，跳过长时间无事实的空桶；时间轴仍保留空桶断点，手动拖动仍可查看空片。
- 相邻时间片切换时预取下一片；请求尚未完成时保留当前轨迹帧并显示“加载下一片…”，不得先清空为 0。新片覆盖物先加入地图，再移除旧片，回放过程中不重复执行视口缩放。
...
| 路网缺失 | 可使用明确标注的“轨迹方位推断”，否则使用“道路 N”或 `unmapped`；不得显示为权威路网匹配 |
...
- 901 px 以上保持地图 + 右栏；900 px 以下右栏移至地图下方，筛选区允许换行；本期不承诺手机端研判。
...
- 允许对缺失 RoadContext 的轨迹做明确标注的 ENU 方位推断，但不得冒充权威道路名称；不对缺失模型来源的历史类别补名称。
