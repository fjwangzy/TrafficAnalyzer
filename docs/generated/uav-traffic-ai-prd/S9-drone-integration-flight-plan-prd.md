...
| 项 | 内容 |
| --- | --- |
| 上位文档 | [无人机交通智能感知系统总 PRD](../2026-06-29-uav-traffic-ai-prd.md) v2.0（2026-07-13） |
| 分册编号 | S9 |
| 产品名称 | 无人机对接与飞行计划管理 |
| 目标路由 | `/drones`（无人机、数据源、飞行计划、执行记录四个页签） |
| 文档状态 | 详细评审草案；设备权威、接口、DDL、权限和调度高可用待冻结 |
| 重要性 / 优先级 | 高 / P0 |
| 需求方 | 交通管理部门 / 投标项目组 |
...
当前平台能够从无人机遥测或管道消息自动形成内存无人机记录，也能通过 `POST /api/v1/missions` 携带视频源、路口和 SRT 文件路径后立即启动检测管道，但尚不能稳定回答以下运营问题：
...
现有 `DRONES`、`MISSIONS` 和 PipelineManager 注册表均以内存为主，平台重启后不可作为业务真源；当前 Mission 创建即运行，也没有 FlightPlan 与一次执行实例的领域区分。上述现状是迁移基线，不是目标契约。
...
| S9-TBD-005 | Drone/Source/FlightPlan/Mission API 精确 schema、错误码、分页、并发控制和版本 | 平台/前端/架构 | OpenAPI 与契约测试 | 【验收阻断】 |
| S9-TBD-006 | `uav_drones/uav_video_sources/uav_telemetry_sources/uav_flight_plans/uav_missions/uav_pipelines` ERD、DDL、迁移和回滚 | DBA/平台/架构 | 批准 migration、对账和回滚报告 | 【验收阻断】 |
| S9-TBD-007 | 调度部署拓扑、advisory lock/租约、节点归属、数据库中断和运行 Pipeline 恢复策略 | 架构/运维/平台 | 高可用设计与故障演练 | 【验收阻断】 |
...
- 进入开发前至少冻结 S9-TBD-001/002/004/005/006/007，并确认本地资产目录和权限。
