...
class PostgresReplayRepository:
    ...
    async def _mission_document(self, intersection_id: str, mission_id: str) -> dict:
            ...
            points_by_track.setdefault(point.track_id, []).append(
                {
                    ...
                    "speed": {
                        ...
                        "enu_vector_mps": (
                            ...
                            if point.velocity_east_mps is not None and point.velocity_north_mps is not None
                            else None
                        ),
            ...
            journeys.append(
                {
                    ...
                    "episodes": [behavior_dict(row) for row in episode_rows if row.track_id == event.track_id],
                    "maneuvers": [behavior_dict(row) for row in maneuver_rows if row.track_id == event.track_id],
    ...
    async def replay(self, intersection_id: str, **kwargs) -> dict:
        document = await self._mission_document(intersection_id, kwargs["mission_id"])
        return await InMemoryReplayRepository([document]).replay(intersection_id, **kwargs)
