...
async def test_spatial_ready_track_query_filters_before_applying_the_limit():
    ...
    assert "jsonb_path_query_array" in where_sql
    assert "@ != null" in where_sql
    assert "AS JSONPATH" in where_sql
    assert ">= 6" in where_sql
...
async def test_track_query_uses_typed_gcj02_and_map_lineage_as_public_authority():
    ...
    row = SimpleNamespace(
        ...
        trajectory_px=[[10.0, 20.0], [12.0, 24.0]],
        ...
        road_match_quality="verified",
        quality_reasons=[],
        geo_registration_id="SGR-1",
    ...
    assert tracks[0]["trajectory_px"] == row.trajectory_px
    assert tracks[0]["association_id"] == "association-1"
